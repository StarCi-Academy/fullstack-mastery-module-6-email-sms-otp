...
export class OtpService {
    ...
    private readonly FAIL_TTL = 900 // 15 phút khóa nếu sai quá nhiá»u (EN: 15 minutes block)
    ...
    async sendOtp(phone: string) {
        ...
        if (retryCount && parseInt(retryCount) >= this.RETRY_LIMIT) {
            ...
            throw new HttpException(
                "Bạn đã gửi yêu cầu quá nhiá»u lần. Vui lòng thử lại sau 1 phút.",
        ...
        if (currentRetry === 1) {
            ...
            await this.redisService.expire(retryKey,
                this.RETRY_TTL)
        }
   ...
   */
    async verifyOtp(phone: string, code: string) {
        ...
        // 1. Kiểm tra xem có bị khóa do nhập sai quá nhiá»u không (EN: Check if blocked due to brute-force)
        ...
        if (!storedOtp || storedOtp !== code) {
            ...
            if (currentFails === 1) {
                ...
                await this.redisService.expire(failKey,
                    this.FAIL_TTL)
            }
